...
Платформа должна работать на нескольких фермах по принципу «центральный сервер — агенты». Каждая ферма должна продолжать локальный мониторинг, управление оборудованием и оповещение оператора при отсутствии интернет-связи, а после восстановления связи синхронизироваться с центральной системой. Поэтому ни одна внешняя корпоративная система не может быть обязательной частью локального операционного пути.
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Мониторинг и оповещения | Платформа выявляет нештатные состояния свиней и оборудования и оповещает Оператора фермы.                                                                                                        |
...
| Локальная работа        | Локальные приложения ферм работают без интернета, сохраняют данные, оповещают дежурного сотрудника и синхронизируются после восстановления связи.                                                |
...
- Платформа мониторинга поголовья MVP содержит центральную систему и локальные приложения ферм. Она владеет локальным операционным путём: обработкой данных устройств, управлением оборудованием, локальными оповещениями, накоплением данных и последующей синхронизацией.
...
[Исходный код диаграммы](./01-primary.c4.context.puml)
...
[Исходный код диаграммы](./02-alternative.c4.context.puml)
...
1. **Недоступность внешних систем или WAN-каналов**  
   _Меры:_ локально сохранять данные, отправлять оповещения Оператору фермы, использовать основной и резервный интернет-каналы и синхронизировать данные после восстановления связи.
